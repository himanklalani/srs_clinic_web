...
const ImageComparisonImage = ({
  ...
  if (isEmpty) {
    return (
      <motion.div
        className={cn(
          "absolute inset-0 h-full w-full flex items-center justify-center bg-[#f5f0ff] border-2 border-dashed border-primary/10 text-primary/30 font-bold tracking-widest text-xs sm:text-sm",
          className
        )}
        style={{
      ...
      >
        <div className="flex flex-col items-center gap-2">
          <span>{position === "left" ? "BEFORE" : "AFTER"}</span>
          <span className="text-[10px] font-normal opacity-60">IMAGE PENDING</span>
        </div>
      </motion.div>
    );
  }
